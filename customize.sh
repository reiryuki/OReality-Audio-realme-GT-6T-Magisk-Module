...
# mode
if [ "`grep_prop oreality.odm $OPTIONALS`" != 1 ]; then
  ui_print "- Changing to /vendor mode instead of /vendor/odm mode"
  ui_print "  Please wait..."
  cp -rf $MODPATH/system/vendor/odm/* $MODPATH/system/vendor
  rm -rf $MODPATH/system/vendor/odm
  mv -f $MODPATH/system/vendor/etc/audio/audiox_param $MODPATH/system/vendor/etc/audio/audioxprm
  FILE=`find $MODPATH -type f -name liboppo_audiox_sw_effects.so`
  sed -i 's|/odm/etc/audio/audiox_param|/vendor/etc/audio/audioxprm|g' $FILE
  sed -i 's|#resetprop -n ro.oplus.audiox.effect.path|resetprop -n ro.oplus.audiox.effect.path|g' $MODPATH/service.sh
else
  ui_print "- Using original /vendor/odm mode"
fi
ui_print " "
...
if [ "`grep_prop oreality.game $OPTIONALS`" != 0 ]; then
  sed -i 's|#p||g' $FILE
  sed -i 's|#g||g' $FILE
else
  ui_print "- Does not use OReality Game patch & rerouting stream"
  ui_print " "
fi
